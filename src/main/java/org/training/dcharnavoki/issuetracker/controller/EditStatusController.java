...
public class EditStatusController extends AbstractBaseController {
	...
	/** The Constant LOG. */
	private static final Logger LOG = Logger.getLogger(EditStatusController.class);

	/** The Constant RESOLUTIONS. */
	private static final String ITEMS = "statuses";
	...
	protected void performTask(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			List<Status> items = factory.getConfDAO().getStatuses();
			request.setAttribute(ITEMS, items);
			jump(Constant.JUMP_JSP_EDIT_STATUS, request, response);
